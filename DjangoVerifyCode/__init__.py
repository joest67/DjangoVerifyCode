...
class Code(object):
    ...
    def display(self):
        ...
        # 随即背景颜色
        self.background = (random.randrange(230, 255), random.randrange(
            230, 255), random.randrange(230, 255))
        ...
        # creat a image
        im = Image.new(
            'RGB', (self.img_width, self.img_height), self.background)
        ...
        for i in range(random.randrange(c - 2, c)):
            line_color = (random.randrange(0, 255), random.randrange(
        ...
        for i in self.code:
            ...
            draw.text(
                (x, y), i, font=self.font, fill=random.choice(self.font_color))
